...
exports.initializeOllama = exports.chatWithOllama = exports.analizarRespuestasOllama = exports.queryOllama = exports.downloadModel = exports.checkOllamaHealth = void 0;
...
const checkOllamaHealth = async () => {
    try {
        ...
        return models.some((model) => model.name.includes(OLLAMA_MODEL.split(':')[0]));
    }
    catch (error) {
 ...
 */
const analizarRespuestasOllama = async (preguntas, respuestas) => {
    try {
    ...
    ${preguntasRespuestas.map(pr => `- Pregunta: "${pr.pregunta}" (Peso: ${pr.peso})
       - Respuesta: ${pr.respuesta}/5`).join('\n\n')}
    ...
    Responde SOLO en este formato JSON:
    {
      ...
      "recomendaciones": ["recomendación específica 1", "recomendación específica 2", "recomendación específica 3"]
        ...
        const response = await (0, exports.queryOllama)(prompt, systemPrompt);
        try {
...
exports.analizarRespuestasOllama = analizarRespuestasOllama;
/**
 * Chat general con IA usando Ollama
 */
const chatWithOllama = async (mensaje, contexto) => {
    try {
...
- Sé conservador y empático en tus respuestas`;
        ...
        const respuesta = await (0, exports.queryOllama)(prompt, systemPrompt);
        return {
            respuesta,
...
const initializeOllama = async () => {
    try {
        ...
        const isHealthy = await (0, exports.checkOllamaHealth)();
        if (!isHealthy) {
            console.log('Modelo no encontrado, descargando...');
            const downloaded = await (0, exports.downloadModel)();
            if (!downloaded) {
...
exports.initializeOllama = initializeOllama;
